...
class NanoTrainer:
    ...
    def _detect_device(self) -> str:
        ...
        try:
            ...
            self._all_devices = dm.all_devices
            logger.info(f"Multi-GPU: {len(self._all_devices)} device(s) available: {self._all_devices}")
            return dm.device
        except ImportError:
            pass
        ...
        if torch.cuda.is_available():
            ...
            self._all_devices = [f"cuda:{i}" for i in range(gpu_count)]
            logger.info(f"Multi-GPU fallback: {gpu_count} CUDA device(s)")
            return "cuda"
        try:
    ...
    async def _train_epoch(self) -> None:
        ...
        for nano_idx, (nano_type, nano) in enumerate(self._nanos.items()):
            try:
                # Distribute nanos across GPUs round-robin
                device = self._all_devices[nano_idx % len(self._all_devices)]
                nano = nano.to(device)
                nano.train()
