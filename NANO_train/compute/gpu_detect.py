...
def _deduplicate_gpus(gpus: List[GPUInfo]) -> List[GPUInfo]:
    ...
    Key includes device_index so identical physical GPUs (e.g. 2x GTX 1660)
    with different indices are kept as separate entries."""
    priority = {
    ...
    seen: Dict[str, GPUInfo] = {}  # key: normalized name + device_index
    for gpu in gpus:
        key = f"{_normalize_gpu_name(gpu.name)}:{gpu.device_index}"
