...
class DeviceManager:
    ...
    def all_devices(self) -> List[str]:
        """Return device strings for ALL available GPUs (for multi-GPU distribution)."""
        if not self._gpus:
            return ["cpu"]
        return [g.torch_device for g in self._gpus]

    def get_device_for_index(self, index: int) -> str:
        """Round-robin device selection across all GPUs."""
