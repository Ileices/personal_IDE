...
export async function conversationIndexRoutes(app: FastifyInstance) {
  ...
  app.post('/index', safeRoute(async (req: FastifyRequest) => {
  ...
  // Search by hotwords
  app.get('/search/:projectId', safeRoute(async (req: FastifyRequest) => {
  ...
  app.get('/files/:projectId', safeRoute(async (req: FastifyRequest) => {
    const { projectId } = req.params as { projectId: string };
  ...
  app.get('/decisions/:projectId', safeRoute(async (req: FastifyRequest) => {
    const { projectId } = req.params as { projectId: string };
  ...
  app.get('/hotwords/:projectId', safeRoute(async (req: FastifyRequest) => {
    const { projectId } = req.params as { projectId: string };
