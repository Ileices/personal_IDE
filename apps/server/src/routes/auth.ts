...
// Simple XOR encryption for token storage (not production-grade, but functional)
const ENCRYPT_KEY = 'personal-ide-local-key-2026';
function xorEncrypt(text: string): string {
  return Buffer.from(
    text.split('').map((c, i) => c.charCodeAt(0) ^ ENCRYPT_KEY.charCodeAt(i % ENCRYPT_KEY.length))
  ).toString('base64');
}
function xorDecrypt(encoded: string): string {
  const buf = Buffer.from(encoded, 'base64');
  return Array.from(buf).map((b, i) => String.fromCharCode(b ^ ENCRYPT_KEY.charCodeAt(i % ENCRYPT_KEY.length))).join('');
}
