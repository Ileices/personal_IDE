      ...
      try {
        ...
        if (limitCheck.isLimit) {
          onProgress?.({
            ...
            chunkIndex: i,
            totalChunks,
            message: `Chunk ${i + 1} still too large. Consider a smaller model context window. Error: ${err.message}`,
          });

          // Try with an even smaller chunk by recursing with halved context
          if (dataTokensPerChunk > 2000) {
