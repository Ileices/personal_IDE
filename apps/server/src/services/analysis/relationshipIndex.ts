  ...
  switch (language) {
    ...
    case 'rust':
      return [
        ...
        { regex: /impl(?:<[^>]*>)?\s+(\w+)/g, kind: 'class', nameGroup: 1 }, // treat impl blocks as class-like
      ];
...
function extractImports(code: string, language: string): ImportInfo[] {
  ...
  switch (language) {
    ...
    default: {
      // Generic: catch import/require/include/use patterns
...
// ── Purity Score Estimation ──

function estimatePurityScore(code: string, symbolName: string, language: string): number {
  // Find the function/method body roughly
  const bodyMatch = new RegExp(`(?:function|def|fn|func)\\s+${symbolName}[^{]*\\{([\\s\\S]*?)\\}`, 'i').exec(code);
  const body = bodyMatch?.[1] || '';

  let score = 1.0;

  // Side-effect indicators lower the score
  const sideEffects = [
...
function detectDomain(filePath: string, symbolName: string): string {
