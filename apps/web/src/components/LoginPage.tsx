...
import { LogIn, Key, AlertCircle, User, Trash2, CheckCircle, ExternalLink, Monitor } from 'lucide-react';

export function LoginPage() {
  const { login, loginAsGuest, switchAccount, removeAccount, accounts, error, isLoading, clearError } = useAuthStore();
  ...
  return (
    <div className="h-full flex items-center justify-center bg-ide-bg">
      <div className="max-w-md w-full mx-4">
        ...
        {/* Guest / Local Mode */}
        <div className="bg-ide-sidebar rounded-lg border border-ide-border p-4 mb-4">
          <button
            onClick={() => loginAsGuest()}
            disabled={isLoading}
            className="w-full bg-ide-accent/20 text-ide-accent font-medium py-3 rounded-lg hover:bg-ide-accent/30 disabled:opacity-50 transition-colors text-sm flex items-center justify-center gap-2 border border-ide-accent/30"
          >
            <Monitor className="w-4 h-4" />
            {isLoading ? 'Starting...' : 'Continue as Guest (Local Mode)'}
          </button>
          <p className="text-xs text-ide-text-dim mt-2 text-center">
            No GitHub account needed. Use with Ollama, LM Studio, or any local provider.
          </p>
        </div>
        ...
        <div className="relative mb-4">
          ...
          <div className="relative flex justify-center text-xs">
            <span className="bg-ide-bg px-2 text-ide-text-dim">or sign in with GitHub</span>
          </div>
        </div>
