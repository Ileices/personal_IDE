...
export function MidwifePanel({ onClose }: Props) {
  ...
  const allModels = MODELS.map(m => ({ id: m.id, name: m.name, publisher: m.publisher }));
      ...
      {history.length === 0 ? (
        ...
        <div className="space-y-1 max-h-[400px] overflow-y-auto">
          {[...history].reverse().map((entry, i) => (
            <div key={i} className="flex items-start gap-2 px-2 py-1.5 text-[10px] border border-ide-border/50 rounded hover:bg-ide-bg/30">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="text-ide-text-dim truncate mt-0.5">{entry.outputSnippet}</div>
              </div>
            </div>
          ))}
        </div>
